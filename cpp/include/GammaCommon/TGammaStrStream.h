...
namespace Gamma
{
		  ...
		  , public std::basic_ostream<_Elem, _Traits >
	{
		...
		const TGammaStrStream& operator=( const TGammaStrStream& );
		TGammaStrStream( const TGammaStrStream& );
		...
		TGammaStrStream( _Str& szString, const _Elem cTag = 0 )
		...
		TGammaStrStream( _Elem* szString, size_t nElemCount, const _Elem cTag = 0 )
			: std::basic_ostream<_Elem, _Traits >( static_cast< std::basic_streambuf<_Elem>* >( this ) )
			  , m_pBuf( szString )
		...
		template<size_t nElemCount>
			TGammaStrStream( _Elem(&szString)[nElemCount] )
			: std::basic_ostream<_Elem, _Traits >( static_cast< std::basic_streambuf<_Elem>* >( this ) )
			  , m_pBuf( szString )
			  , m_nElemCount( nElemCount )
			  , m_nCurPos( 0 )
		...
		template<class Type>
		TGammaStrStream<_Elem>& operator << ( Type&& _Val )
		{
		...
		template<class Type>
		TGammaStrStream<_Elem>& operator << ( const Type& _Val )
		{
			std::basic_ostream<_Elem, _Traits >& _this = *this;
			_this << _Val;
			if( m_charTag )
		...
		TGammaStrStream<_Elem>& operator << ( std::basic_ostream<_Elem>& (fun)( std::basic_ostream<_Elem>& ) )
		{
			std::basic_ostream<_Elem, _Traits >& _this = *this;
			_this << fun;
			return *this;
		...
		template<class Type>
		TGammaStrStream<_Elem>& operator << (const SValueFormat<Type>& fmtValue)
		{
			std::basic_ostream<_Elem, _Traits >& _this = *this;
		...
		TGammaStrStream<_Elem>& operator << (const _Elem* _Val)
		{
			return *this << TConstString<_Elem>(_Val, true);
		}
	...
	typedef TGammaStrStream<char>		gammasstream;
	typedef TGammaStrStream<wchar_t>	wgammasstream;
