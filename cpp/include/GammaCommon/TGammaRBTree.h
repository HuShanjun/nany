...
namespace Gamma
{
	...
	class TGammaRBTree
	{
		const TGammaRBTree& operator= ( const TGammaRBTree& );
		TGammaRBTree( const TGammaRBTree& );
		enum { eRed, eBlack, eDoubleBlack };
	public:
		typedef ImpClass CNode;
		typedef TGammaRBTree<CNode> CGammaRBTree;
		...
		class CGammaRBTreeNode
		{
			...
			union 
			{
				CGammaRBTreeNode*	m_pParent;
				CGammaRBTree*		m_pTree;
			};

			friend class TGammaRBTree<ImpClass>;

			// 红黑树的左旋
			void LeftRotate()
			{
				GammaAst( m_pRightChild );
				if( m_bRootNode )
				...
				CGammaRBTreeNode* pRight = m_pRightChild;
			...
			void RightRotate()
			{
				...
				CGammaRBTreeNode* pLeft = m_pLeftChild;
			...
			void ReplaceBy( CGammaRBTreeNode* pNode )
			{
				CGammaRBTreeNode* pParent = m_pParent;
		...
		public:
			...
			void Remove()
			{
				...
				// 哨兵节点，临时用
				struct CSentryNode : public CGammaRBTreeNode
				{
					~CSentryNode()
					{
						if( !m_pParent )
							return;
						CGammaRBTreeNode* pCurNode = this;
						GammaAst( m_nNodeColor == eBlack );
						GammaAst( !m_pLeftChild && !m_pRightChild );
						if( m_bRootNode )
				...
				{
					CGammaRBTreeNode* pNext = GetNext();

					// 调换位置，但是保留CGammaRBTreeNode的属性;
				...
				CGammaRBTreeNode* pParent = m_pParent;
				CGammaRBTreeNode* pNode = m_pLeftChild ? m_pLeftChild : m_pRightChild;
				if( !pNode )
					pNode = &SentryNode;
				...
				while( !pNode->m_bRootNode && pNode->m_nNodeColor == eDoubleBlack )
				{
					...
					{
						CGammaRBTreeNode* pBrother = pParent->m_pRightChild;
					...
					{
						CGammaRBTreeNode* pBrother = pParent->m_pLeftChild;
			...
			CNode* GetPre() const
			{
				if( !m_pLeftChild )
				{
					const CGammaRBTreeNode* pNext = this;
					CGammaRBTreeNode* pParent = pNext->GetParent();
				...
				CGammaRBTreeNode* pNext = m_pLeftChild;
			...
			CNode* GetNext() const
			{
				if( !m_pRightChild )
				{
					const CGammaRBTreeNode* pPre = this;
					CGammaRBTreeNode* pParent = pPre->GetParent();
				...
				CGammaRBTreeNode* pPre = m_pRightChild;
	...
	public:
		class iterator
		{
			CGammaRBTreeNode* m_pNode;
		public:
			iterator() : m_pNode( NULL ){}
			iterator( CGammaRBTreeNode* pNode ) : m_pNode( pNode ){}
			iterator( const iterator& rhs ) : m_pNode( rhs.m_pNode ){}
			iterator operator= ( CGammaRBTreeNode* pNode ) { m_pNode = pNode; return *this; }
			iterator operator= ( const iterator& rhs ) { m_pNode = rhs.m_pNode; return *this; }
			bool operator == ( CGammaRBTreeNode* pNode ) const { return m_pNode == pNode; }
		...
		CNode& Insert( CNode& ImpNode )
		{
			auto& Node = static_cast<CGammaRBTreeNode&>(ImpNode);
			...
			CGammaRBTreeNode* pNode = m_pRootNode;

			// 插入二叉查找树
			while( true )
			{
				if( ImpNode < *static_cast<CNode*>( pNode ) )
				{
					if( !pNode->m_pLeftChild )
					{
						pNode->m_pLeftChild = &Node;
						static_cast<CGammaRBTreeNode&>( Node ).m_pParent = pNode;
						break;
					}
			...
			pNode = &Node;
			CGammaRBTreeNode* pParent = static_cast<CGammaRBTreeNode&>( Node ).m_pParent;
			// 如果父节点和当前节点都为红色，则纠正
			while( pNode && pNode->m_nNodeColor == eRed && pParent->m_nNodeColor == eRed )
			{
				CGammaRBTreeNode* pGrandpa = pParent->m_pParent;
				CGammaRBTreeNode* pUncle = pGrandpa->m_pLeftChild == pParent ? 
		...
		CNode* LowerBound( const CNode& key ) const
		{
			if( !m_pRootNode )
				return NULL;
			...
			while( true )
			{
				if( key < *static_cast<CNode*>(pNode) )
				{
					if( !pNode->m_pLeftChild )
						return static_cast<CNode*>(pNode);
					pNode = pNode->m_pLeftChild;
		...
		CNode* Find( const CNode& key ) const
		{
			CNode* pNode = LowerBound( key );
			return !pNode || key < *pNode ? NULL : pNode;
		}
	...
	class TGammaRBSetNode : 
		public TGammaRBTree< TGammaRBSetNode<DataType> >::CGammaRBTreeNode
	{
	protected:
		DataType	m_Data;
	public:
		TGammaRBSetNode( const DataType& Data = DataType() ) : m_Data( Data ){}
	...
	class TGammaRBMapNode : 
		public TGammaRBTree< TGammaRBMapNode<KeyType, DataType> >::CGammaRBTreeNode
	{
	protected:
		KeyType		m_Key;
		DataType	m_Data;
