...
namespace Gamma
{
	inline void sha1( const tbyte* pBuffer, uint32_t nSize, tbyte* sha1 )
	{
		uint32_t SHA1_tmp;
	...
	/******************************************************/
	inline void hmac_sha1( const tbyte *key, const uint32_t key_length, 
		const tbyte *data, const uint32_t data_length, tbyte* digest )
	{
	...
	inline void hmac_sha1_ex( const tbyte* szKey, const uint32_t nKeySize, 
		const tbyte* pData, const uint32_t nDataSize, char* szResult )
	{
		tbyte arySHA1[20];
