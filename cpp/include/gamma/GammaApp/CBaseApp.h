...
{	
	...
	protected:
		...
		CBaseApp(
			const char* szConfigFile = "config.toml",
			uint32 nMaxLoopInterval = BASE_FRAME_INTERVAL,
			bool bLogFile = true, bool bEnableDump = true);
		virtual ~CBaseApp();
	...
	public:
		...
		void				Register( CTick* pTick, uint32 uCyc, uint16 nTickID ) override { Register(pTick, uCyc, uCyc, nTickID); }
