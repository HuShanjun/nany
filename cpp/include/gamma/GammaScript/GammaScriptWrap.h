...
namespace Gamma
{
	...
	class TConstruct : public IObjectConstruct
	{
		...
		void SetVTable( TConstructParams<Param...>*, ClassType* pObj )
		{
			...
			if( !GetVTableType::GetFunInst() )
				return;
			( ( Gamma::SVirtualObj* )pObj )->m_pTable = GetVTableType::GetVTbInst();
		}
	...
	class CScriptRegisterNode : public TGammaList<CScriptRegisterNode>::CGammaListNode
	{
		void( *m_funRegister )( );
		typedef typename TGammaList<CScriptRegisterNode>::CGammaListNode ParentType;
	public:
		CScriptRegisterNode( TGammaList<CScriptRegisterNode>& list, void( *fun )() )
			: m_funRegister( fun )
		{
			list.PushBack( *this );
		}
	...
	typedef TGammaList<CScriptRegisterNode> CScriptRegisterList;
	...
	public:
		static void Bind()
		{
			...
			STypeInfoArray TypeInfo = { aryInfo, sizeof( aryInfo )/sizeof( STypeInfo ) };
			Gamma::CScriptBase::RegisterDestructor(&s_instance, funBoot, GetCallBackIndex(), TypeInfo );
		}
	};
	...
	public:
		...
		{
			GammaAst( !m_pScript == !m_pScriptObject );
			if( !m_pScript )
				return;
