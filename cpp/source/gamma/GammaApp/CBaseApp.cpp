...
namespace Gamma
{
	...
	using namespace std;
	CBaseApp::CBaseApp(const char* szConfigFile, uint32 nMaxLoopInterval,
					bool bLogFile, bool bEnableDump )
		: m_pTickMgr( NULL )
		, m_bQuit(false)
		, m_bSignal(false)
...
#endif
			SIGILL, SIGSEGV, SIGABRT, SIGBUS, SIGTRAP, SIGXCPU, SIGXFSZ
#endif
		};
	...
	void CBaseApp::AppLog( const char* szLog, size_t nLen, uint32 nLevel)
	{
		if( Inst() && !Inst()->m_bQuit )
		{
			...
			sprintf( szBuf, "[%4d-%02d-%02d_%02d:%02d:%02d-%10d][%s]", tmCurTime.tm_year + 1900, 
				tmCurTime.tm_mon + 1, tmCurTime.tm_mday, tmCurTime.tm_hour, tmCurTime.tm_min, tmCurTime.tm_sec, (uint32)GetProcessTime(), pStrLevel[nLevel].c_str() );
	...
	void CBaseApp::SigHandler( int32 nSignum, void* pContext )
	{
		...
		std::string strDumpFileName = std::format("{}_{}.{}.{}.{}.{}_{}_{}-{}-{}-{}.{}.{}.core", 
			m_strClassName,
			version.GetReserve(), version.GetMajor(), version.GetMinor(),
			version.GetRevision(), version.GetBuild(),
			GammaGetCurrentProcessID(),
			tmCurTime.tm_year + 1900, tmCurTime.tm_mon + 1, tmCurTime.tm_mday,
	...
	{
		GammaLog << "CBaseApp::OnPreStart() end......\t" 
			<< m_nLogicTime << endl;	
		m_bStarted = true;
		m_CompMgr.OnStarted();
		OnStarted();
	...
	void CBaseApp::Clearup()
	{
		GammaLog << "Clearup FileMgr" << endl;
		m_CompMgr.Clearup();
	...
	void CBaseApp::Run()
	{
		...
		OnQuit();
		GammaLog << "CBaseApp::Clearup() end......" << endl;
	...
	void CBaseApp::LogProfile()
	{
		...
		for( uint32 i = nStarFrame; i <= nEndFrame; i++ )
		{
			...
			for( size_t j = 0; j < nCount; j++ )
			{
				...
				strStream << "\t\t<" << pProfile->GetLabel() << " t=\'" << nCostTime << "\' n=\'" << nHitCount << "\'/>\n";
			}
			strStream << "\t</frame>\n";
		}
		strStream << "</root>\n";
		...
		time( &nCurTime );
		char szName[256];
		gammasstream( szName, ELEM_COUNT( szName ) ) << GetLogPath()
			<< m_strClassName << "_profile_" << GammaGetCurrentProcessID() 
			<< "_" << nCurTime << ".xml";
    ...
    bool CBaseApp::HasAppSetting(const char *szSection, const char *szKey) const {
        return m_AppConfig.contains(szSection) && m_AppConfig[szSection].as<toml::table>()->contains(szKey);
    }
