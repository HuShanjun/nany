...
namespace Gamma
{
	...
	void* s_szValue2ID = (void*)"v2i";
	void* s_szID2Value = ( void* )"i2v";
	...
	void CDebugLua::StackDump( const char* szTitle, std::ostream& sDebug )
	{
		if (!szTitle)
			szTitle = "Default";

		int top = lua_gettop(m_pState);
		sDebug << szTitle << ":stack num=" << top << std::endl;
		for (int i = top; i > 0; --i)
		{
			...
			{
			case LUA_TSTRING:
				sDebug << "string:" << lua_tostring(m_pState, i) << std::endl;
				break;
			case  LUA_TBOOLEAN:
				sDebug << "bool:" << lua_toboolean(m_pState, i) << std::endl;
				break;
			case LUA_TNUMBER:
				sDebug << "number:" << lua_tonumber(m_pState, i) << std::endl;
				break;
			default:
				sDebug << lua_typename(m_pState, t) << std::endl;
				break;
			}
		}
	}
		...
		if( pInfo->m_mapFields[0].IsEmpty() && pInfo->m_mapFields[1].IsEmpty() )
		{
			...
			else if( lua_isfunction( m_pState, -1 ) )
			{
				for (int i = 0; i < 256; i++)
				{
					...
					char szIndex[256];
					gammasstream(szIndex) << "upvalue_" << i + 1;
