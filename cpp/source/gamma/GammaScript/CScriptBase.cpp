...
namespace Gamma
{
	...
	{
		GammaAst( CClassInfo::GetClassInfo( szTypeIDName ) );
	...
	{
		if( bError )
			GammaErr << ( szBuffer ? szBuffer : "(null c string)" );
		else
			GammaLog << ( szBuffer ? szBuffer : "(null c string)" );
		return nCount;
	}
	...
	void CScriptBase::CallBack( int32_t nIndex, void* pRetBuf, void** pArgArray )
	{
		...
		GammaAst( pFunTableHead->m_pClassInfo && pFunTableHead->m_pOldFunTable );
