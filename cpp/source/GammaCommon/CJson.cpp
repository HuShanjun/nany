...
namespace Gamma
{
	...
	void CJson::Clear()
	{
		if (this == nullptr)
			return;
		while( TGammaList<CJson>::GetFirst() )
			delete TGammaList<CJson>::GetFirst();
	...
	bool CJson::Load( const wchar_t* szFileName )
	{
		CPkgFile JsonFile;
		if ( !JsonFile.Open( szFileName ) )
			return false;
		if ( JsonFile.Size() == 0 )
			return false;
	...
	bool CJson::Load( const char* szFileName )
	{
		CPkgFile JsonFile;
		if ( !JsonFile.Open( szFileName ) )
			return false;
	...
	bool CJson::Save( const wchar_t* szFileName ) const
	{
#ifdef _WIN32
	...
	bool CJson::Save( const char* szFileName ) const
	{
#ifdef _WIN32
		std::ofstream os( Utf8ToUcs( szFileName ).c_str(), std::ios::binary );
#else
		std::ofstream os( szFileName, std::ios::binary );
#endif
		return Save( os, 0 );
	...
	volatile Gamma::CJson* CJson::m_pInvalid = nullptr;
	...
	bool CJson::Parse( CRefStringPtr& DomXmlBuffer, size_t& nCurPos, bool bWithName )
	{
		...
		while( NextNode( nCurPos, c != '[' ) )
		{
			...
			TGammaList<CJson>::PushBack( *pChild );
			m_nChildCount++;
	...
	{
		if(this == nullptr || !szChildName || !szChildName[0])
			return nullptr;
		CJson* pChild = TGammaList<CJson>::GetFirst();
	...
	{
		if(this == nullptr || !szChildName || !szChildName[0] || TGammaList<CJson>::IsEmpty())
			return nullptr;
		CJson* pChild = TGammaList<CJson>::GetFirst();
		while( pChild && strcmp( pChild->GetName(), szChildName ) )
			pChild = pChild->GetNext();
	...
	CJson* CJson::GetNext() 
	{
		return TGammaList<CJson>::CGammaListNode::GetNext();
	}
	...
	CJson* CJson::AddChild( CJson* pChild, CJson* pBefore )
	{
		...
		GammaAst( ( m_szContent == &s_nArrayFlag ) == !( pChild->GetName()[0] ) );
		TGammaList<CJson>::InsertBefore( *pChild, pBefore );
		pChild->SetLevel( m_nLevel + 1 );
		return pChild;
	}
	...
	const CJson& CJson::operator=(const CJson& rhs)
	{
		...
		CJson* pChild = rhs.TGammaList<CJson>::GetFirst();
		while( pChild )
		{
			CJson* pNewChild = new CJson;
			pNewChild->m_nLevel = m_nLevel + 1;
			TGammaList<CJson>::PushBack( *pNewChild );
			*pNewChild = *pChild;
			pChild = pChild->GetNext();
		}
		return *this;
