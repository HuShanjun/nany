...
struct std::formatter<SServerInfo> {
    ...
    auto format(const SServerInfo& serverInfo, std::format_context& ctx) const {
        return std::format_to(ctx.out(), 
                        ...
                        serverInfo.strLocalHost, serverInfo.nLocalPort, serverInfo.strPublicHost, serverInfo.nPublicPort);
    }
};
...
void CNetComp::OnInit()
{
    ...
    for (auto& [key, value] : m_ServerConfig) {
        ...
        std::string strPublicHost = pServerInfo.contains("PublicHost") ? pServerInfo.at("PublicHost").value_or("") : "";
        uint16 nPublicPort = pServerInfo.contains("PublicPort") ? pServerInfo.at("PublicPort").value_or(0) : 0;
        ...
        if (setNetTopology.contains((EServerType)nServerType)) {
            if (nServerType != nCurServerType || nServerType == nCurServerType && nServerID > nCurServerID) {
...
void CNetComp::OnStarted(){
    ...
    if (m_pCurServerInfo && m_pCurServerInfo->nPublicPort > 0) {
        pConnMgr->StartService(m_pCurServerInfo->strPublicHost.c_str(), m_pCurServerInfo->nPublicPort, CGameConnServer::GetID());
        Log::Info("CNetComp::OnStarted StartService {}:{}", m_pCurServerInfo->strPublicHost, m_pCurServerInfo->nPublicPort);
    }
...
void CNetComp::OnReconnectTick()
{
    ...
    for (auto& [nServerID, addr] : m_mapServers) {
        ...
        CBaseConn* pBase = pConnMgr->Connect(addr.GetAddress(), addr.GetPort(), CGameConnServer::GetID());
        if (!pBase) {
